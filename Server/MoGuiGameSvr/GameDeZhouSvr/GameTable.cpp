...
void CGameTable::CeShiInfo(const char* logstr,...)
{
	if ( m_pGSConfig->CeshiMode() )
	{
		...
		if (len>0 && len<=MAX_LOG_BUF_SIZE )
		{
			string strTemp = "TableID=" + N2S(m_RoomID*10000+m_TableID,10) + " " + string(logbuf);
...
int CGameTable::InitTable(CreateTableInfo& cti)
{
	...
	m_TableMatchState           = MatchTable_ST_None;
	...
	m_TenMinuteTime = UINT32(time(NULL)) + Tool::CRandom::Random_Int(200,600);
...
int CGameTable::InitTableRule()
{
	...
	//cti.m_TableRule = "tabletype=0;servicepay=1;smallblind=5;bigblind=10;mintake=20;maxtake=200;minforgame=20;chiptime=15;waittime=3;maxin=250;maxsit=9;minplayer=2;pot=0;chip1=0;chip2=0;chip3=0;chip4=0";
	...
	itorRule = mapRule.find("maxin");
	if ( itorRule != mapRule.end())
	{
		m_MaxInPlayerNumber = UINT16(itorRule->second);
	}

	m_MaxSitPlayerNumber = MAX_PALYER_ON_TABLE;
	itorRule = mapRule.find("maxsit");
	if ( itorRule != mapRule.end())
	{
		m_MaxSitPlayerNumber = BYTE(itorRule->second);
	}
	assert(m_MaxSitPlayerNumber>=2 && m_MaxSitPlayerNumber<=MAX_PALYER_ON_TABLE);

	m_EnterLevel = 0;
	itorRule = mapRule.find("enterlevel");
	if ( itorRule != mapRule.end())
	{
		m_EnterLevel = BYTE(itorRule->second);
	}

	m_EnterMoney = 0;
	itorRule = mapRule.find("entermoney");
	if ( itorRule != mapRule.end())
	{
		m_EnterLevel = BYTE(itorRule->second);
	}

	mapRule = Tool::GetRuleMapInData(m_TableRuleEX,string(";"),string("="));

	m_RuleShow = 0;
	itorRule = mapRule.find("show");
	if ( itorRule != mapRule.end() )
	{
		m_RuleShow = INT8(itorRule->second);
	}
...
BYTE CGameTable::GetEmptySitID(BYTE FromSitID) const
{
...
bool CGameTable::IsTableMatching()const
{
	return m_pGameLogic->IsMatching();
}
bool CGameTable::IsMatchTable()const
{
	return m_pGameLogic->IsMatchTable();
}
...
int CGameTable::AddBotPlayer(INT64 nBigBlind)
{
	...
	BYTE SitID = GetEmptySitID(Tool::CRandom::Random_Int(1,m_MaxSitPlayerNumber));
	if ( SitID && BotPlayer )
	{
		bool bSuccess = false;
		if ( m_pRoom->PlayerJoinRoom(BotPlayer) && PlayerJoinTable(BotPlayer) )
		{
			...
			INT64 nTakeMoney = 0;
			if ( BotPlayer->GetGameMoney()/5 >= m_pGameLogic->m_nMaxTakeIn/2 )
			{
				nTakeMoney = m_pGameLogic->m_nMaxTakeIn/2;
			}
			else if( BotPlayer->GetGameMoney()/5 >= nBigBlind*200 )
			{
				nTakeMoney = nBigBlind*200;
			}
			else
			{
				nTakeMoney = max( (BotPlayer->GetGameMoney()/(nBigBlind*5))*nBigBlind,m_pGameLogic->m_nMinTakeIn);
			}

			if ( PlayerSitDown(BotPlayer,SitID,nTakeMoney,Action_Flag_SysLogic) )
			{
				bSuccess = true;
...
int CGameTable::RemoveBotPlayer(UINT32 PID)
{
	...
	CeShiInfo("CGameTable::RemoveBotPlayer PID=%d",PID);
...
bool CGameTable::OnTimer(UINT32 curTime)
{
	...
	if ( m_TableCurTime >= m_TenMinuteTime )
	{
		m_TenMinuteTime = m_TableCurTime + Tool::CRandom::Random_Int(400,800);
		for (MapPlayer::iterator itorPlayer=m_TablePlayers.begin();itorPlayer!=m_TablePlayers.end();++itorPlayer)
		{
	...
	if ( m_TimeToClearTable>0 && curTime>m_TimeToClearTable )
	{
		m_TimeToClearTable = 0;
		if ( IsMatchTable() )
		{
			ClearAllPlayer();
			m_pGameLogic->EndGameLogic();
		}		
	}
...
UINT32 CGameTable::GetTime() const
{
	return m_TableCurTime;
}

int CGameTable::GetPlayerTableMoney(vector<PlayerPtr>& vecPlayer,vector<INT64>& vecMoney)
{
	for (int SitID=1;SitID<=m_MaxSitPlayerNumber;++SitID)
	{
		MapSitPlayer::const_iterator itorSitPlayer = m_SitPlayers.find(SitID);
		if ( itorSitPlayer != m_SitPlayers.end() )
		{
			vecPlayer.push_back(itorSitPlayer->second);

			INT64 nTableMoney = m_pGameLogic->GetPlayerRealTableMoneyBySitID(SitID);
			assert(nTableMoney>0);
			vecMoney.push_back(nTableMoney);
		}
	}
	return 0;
}

void CGameTable::OnChangeBlind(int BigBlind,int SmallBlind)
{
	assert(m_pGameLogic->IsRoomMatch());
	m_pGameLogic->SendChangeBlind(BigBlind,SmallBlind);
}

void CGameTable::InitMatchTableData()
{
	m_pGameLogic->InitMatchData();
}
...
bool CGameTable::PlayerSitDown(PlayerPtr pPlayer,int SitID,INT64 nTakeMoney,int Flag)
{
	...
	if ( pPlayer && IsAvailSitID(SitID) && pPlayer->GetTableID()==m_TableID )
	{
		...
		if ( m_pGameLogic->IsMatchTable() )
		{
			m_pGameLogic->MatchPlayerSitDown(pPlayer,SitID,nTakeMoney,Flag);
		}
		else
		{
...
bool CGameTable::PlayerStandUp(PlayerPtr pPlayer,int SitID,int Flag)
{
		...
		&& pPlayer->GetTableID() == m_TableID && pPlayer->GetSitID() == SitID )
	{
		...
		if (m_pGameLogic->IsMatchTable())
		{
			m_pGameLogic->MatchPlayerStandUp(pPlayer,SitID,Flag);
		}
		else
		{
	...
	if ( pPlayer && pPlayer->GetTableID()==this->m_TableID && pPlayer->GetRoomID() == this->m_RoomID )
	{
		if ( pPlayer->IsPlayerSitOnTable() )
		{
			if( GetSitPlayerBySitID(pPlayer->GetSitID()) == pPlayer )
			{
				...
				if ( m_pGameLogic->IsMatchTable() )
				{
					m_pGameLogic->MatchPlayerStandUp(pPlayer,pPlayer->GetSitID(),Flag);
				}
				else
				{
		...
		if ( CGameRoom* pRoom = pPlayer->GetRoom() )
		{
			pRoom->SendTablePlayerCount(pPlayer);
		}
...
int CGameTable::OnReqPlayerAct(PlayerPtr pPlayer,CRecvMsgPacket& msgPack)
{
		...
		&& rpa.m_TableID == pPlayer->GetTableID() && m_RoomID == pPlayer->GetRoomID() )
	{
		...
		if( IsAvailSitID(rpa.m_SitID) )
		{
			...
			else if ( rpa.m_Action == rpa.CHANGESIT )
			{
				if ( IsAvailSitID(pPlayer->GetSitID()) && IsAvailSitID(rpa.m_SitID)
					&& pPlayer->GetSitID()!=rpa.m_SitID && IsSitIDEmpty(rpa.m_SitID) )
				{
					if ( m_pGameLogic->IsCommondTable() )
					{
						if ( m_pGameLogic->IsGamePlaying() )
						{
							int nGameState = m_pGameLogic->GetSitPlayerState(pPlayer->GetSitID(),pPlayer->GetPID());
							if ( (nGameState==GAME_PLAYER_ST_WAITING || nGameState==GAME_PLAYER_ST_GIVEUP) )
							{
								rspa.m_Flag = rspa.SUCCESS;
							}
							else
							{
								rspa.m_Flag     = rspa.Gameing;
								rspa.m_ShowFlag = ShowMsg_Text;
								rspa.m_strMsg   = "游戏中不能换位置";
							}
						}
						else
						{
					...
					{
						rspa.m_Flag     = rspa.Match;
						rspa.m_ShowFlag = ShowMsg_Show;
						rspa.m_strMsg   = "比赛桌不能换位置";
					}
				}
			}
		}
		...
		if( rspa.m_Flag == rspa.SUCCESS )
		{
			...
			{
				INT64 nTableMoney = m_pGameLogic->GetPlayerRealTableMoneyBySitID(pPlayer->GetSitID());

				PlayerStandUp(pPlayer,pPlayer->GetSitID());
				PlayerSitDown(pPlayer,rpa.m_SitID,nTableMoney);
			}
		}
	}
	else
