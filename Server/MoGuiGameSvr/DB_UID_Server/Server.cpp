...
int CDBMsgThread::Run( void )
{
	...
	CDBOperator *pDoDBMsg    = new CDBOperator(m_pDBServer);
...
CServer::CServer()
{
	...
	m_pDBOperator = new CDBOperator(this);
...
void CServer::DebugError(const char* logstr,...)
{
	...
	if( len>0 && len<=MAX_LOG_BUF_SIZE )
	{
		...
		std::cout<<"Error:DBCServer "<<logbuf<<endl;
	}
}
...
void CServer::CeShiInfo(const char* logstr,...)
{
#ifdef CESHI
	static char logbuf[MAX_LOG_BUF_SIZE+1] = {0};
	va_list args;
...
void CServer::OnTenMinite(UINT32 curTime)
{
	...
	{
		int nUseTime = curTime - CDBOperator::s_LastTime;
		DebugLogOut("Read  Total=%-10s Speed=%-5d",Tool::N2S(CDBOperator::s_ReadOperator).c_str(),
			int((CDBOperator::s_ReadOperator-CDBOperator::s_LastReadOperator)/nUseTime));

		DebugLogOut("Write Total=%-10s Speed=%-5d",Tool::N2S(CDBOperator::s_WriteOperator).c_str(),
			int((CDBOperator::s_WriteOperator-CDBOperator::s_LastWriteOperator)/nUseTime));

		CDBOperator::s_LastTime = curTime;
		CDBOperator::s_LastReadOperator  = CDBOperator::s_ReadOperator;
		CDBOperator::s_LastWriteOperator = CDBOperator::s_WriteOperator;
...
void CServer::CheckPlayerOnLine(UINT32 curTime)
{
	...
	UINT32 MaxExistTime = UINT32(INT64(Max_PlayerData_In_DBS)*N_Time::Day_1/m_PlayerInfos.size());
...
void CServer::CheckBotPlayerCached(UINT32 curTime)
{
	...
	if ( m_CurTime - m_CheckBotPlayerCached >= nIntervalTime )
	{
		...
		m_CheckBotPlayerCached = curTime;
...
INT16 CServer::GetAIDByPID(UINT32 PID)
{
	INT16 AID = 0;
	if (PID==0) return AID;

	MapPIDAID::iterator itorAID = m_mapAIDPID.find(PID);
	if ( itorAID != m_mapAIDPID.end() )
	{
		AID = itorAID->second;
	}
	else
	{
		...
		if ( pPlayerInfo )
		{
			AID = pPlayerInfo->m_AID;
		}
		else
		{
			AID = m_pDBOperator->ReadUserAID(PID);
		}
		if ( AID>0 ) m_mapAIDPID.insert(make_pair(PID,AID));
	}
	return AID;
}
...
int CServer::InitBaseInfo()
{
	...
	if ( vectorAI.size() )
	{
		...
		{
			m_mapAreaInfo.insert(make_pair(vectorAI[nPos].m_AID,vectorAI[nPos]));
		}
	}
...
int CServer::OnGameServerMsg( CDBServerSocket* pDBSocket,CRecvMsgPacket& msgPack )
{
	...
	case DBServerXY::WDB_ReportPlayerOnline::XY_ID:
		...
		break;
	case DBServerXY::ReqChangeBankMoney::XY_ID:
		{
			ret = OnWDBChangeBankMoney(pDBSocket,msgPack);
		}
		break;
	...
	case DBServerXY::WDB_ReqFinishHonor::XY_ID:
		{
			ret = OnWDBFinishHonor(pDBSocket,msgPack);
		}
		break;
...
int CServer::OnReqServerConnect( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	{
		if ( rsc.m_ClientClass == CLIENT_CLASS_GAMESERVER )
		{
			//此处应该加上验证SESSION的内容

			pSocket->SetClass(rsc.m_ClientClass);
...
int CServer::OnReqBotPlayerInfo( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	if ( msgRbp.m_ServerID==pSocket->GetServerID() && msgRespBP.m_Flag==msgRespBP.SUCCESS )
	{
		...
		for ( VectorBotPlayerData::iterator itorPlayerData=vectorPD.begin();itorPlayerData!=vectorPD.end();++itorPlayerData )
		{
			...
			BotPlayerData.m_JoinTime              = itorPlayerData->m_stUserDataInfo.m_JoinTime;
			...
			BotPlayerData.m_nMoGuiMoney           = itorPlayerData->m_stGameInfo.m_nMoGuiMoney;
			BotPlayerData.m_nMatchJF              = itorPlayerData->m_stGameInfo.m_nMatchJF;		
			...
			BotPlayerData.m_nUpperLimite          = itorPlayerData->m_stGameInfo.m_nUpperLimite;
			BotPlayerData.m_nLowerLimite          = itorPlayerData->m_stGameInfo.m_nLowerLimite;

			BotPlayerData.m_RightLevel            = PLAYER_RIGHT_COMMON;
			BotPlayerData.m_RightTimes            = 255;

			BotPlayerData.m_nWinTimes             = BotPlayerData.m_nWinTimes/10;
			BotPlayerData.m_nLossTimes            = BotPlayerData.m_nLossTimes/10;

			BotPlayerData.m_MaxPai                = itorPlayerData->m_stGameInfoEX.m_MaxPai;
			BotPlayerData.m_MaxPaiTime            = itorPlayerData->m_stGameInfoEX.m_MaxPaiTime;
			BotPlayerData.m_MaxMoney              = itorPlayerData->m_stGameInfoEX.m_MaxMoney;
			BotPlayerData.m_MaxMoneyTime          = itorPlayerData->m_stGameInfoEX.m_MaxMoneyTime;
			BotPlayerData.m_MaxWin                = itorPlayerData->m_stGameInfoEX.m_MaxWin;
			BotPlayerData.m_MaxWinTime            = itorPlayerData->m_stGameInfoEX.m_MaxWinTime;

			memcpy(BotPlayerData.m_WinRecord,itorPlayerData->m_stGameInfoEX.m_WinRecord,sizeof(BotPlayerData.m_WinRecord));
...
int CServer::OnReqRoomTableInfo( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	if ( TempTableInfoVector.size()>0 && TempRoomInfoVector.size()>0)
	{
		DBServerXY::DBS_RespRoomTableInfo rspRTI;
		...
		for ( unsigned int i=0;i<TempTableInfoVector.size();++i)
		{
			if ( TempTableInfoVector[i].m_ServerID == pSocket->GetServerID() )
			{
				...
				ti.m_TableRuleEX      = TempTableInfoVector[i].m_TableRuleEX;
				ti.m_MatchRule        = TempTableInfoVector[i].m_MatchRule;
...
INT64 CServer::GetPlayerRight(DBServerPlayerInfo* pPlayerInfo)
{
	...
	m_pDBOperator->ReadUserRight(pPlayerInfo->m_AID,pPlayerInfo->m_PID,vecRight);
	if ( vecRight.size() )
	{
		vector<int>  vecDelIdx;
		for ( size_t nPos=0;nPos<vecRight.size();++nPos)
		{
			stDBGameRight& rGR = vecRight[nPos];

			vecDelIdx.push_back(rGR.m_Idx);
			if ( rGR.m_EndTime >= m_CurTime	&& rGR.m_Level>PLAYER_RIGHT_NONE && rGR.m_Level<=PLAYER_RIGHT_KING )
			{
				pPlayerInfo->m_RightInfo.AddRight(rGR.m_Level,rGR.m_Times);
			}
		}

		for ( size_t nPos=0;nPos<vecDelIdx.size();++nPos)
		{
			RWDB_UpdateRightInfo rwdbURI;
			rwdbURI.m_RightIdx = vecDelIdx[nPos];
			rwdbURI.m_CanUse   = 0;
			m_RWDBMsgManager.PushRWDBMsg(rwdbURI);
		}
	}
	
	DebugLog("GetPlayerRight End PID=%d ",pPlayerInfo->m_PID);
	return pPlayerInfo->m_RightInfo.GetRight();
}
...
int CServer::OnReqPlayerLogin(CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	DebugLog("OnReqPlayerLogin 认证 AID=%d PID=%d Seesion=%s",msgLogin.m_AID,msgLogin.m_PID,msgLogin.m_Session.m_Session);
	...
	{
		string strSession = "";
		if ( m_memOperator.GetSession(msgLogin.m_AID,msgLogin.m_PID,strSession))
		{
			DebugLog("OnReqPlayerLogin PID=%d getSS=%s msgSS=%s",msgLogin.m_PID,strSession.c_str(),msgLogin.m_Session.GetString().c_str());
			if ( strSession == msgLogin.m_Session.GetString() )
			{
				msgBackData.m_Flag = msgBackData.SUCCESS;
	...
	if ( N_CeShiLog::c_NoCheckLogin && msgLogin.m_AID==N_AREAID::AREAID_CESHI )
	{
		msgBackData.m_Flag = msgBackData.SUCCESS;
	}

	if ( msgLogin.m_AID==1 && msgLogin.m_PID<=1000 )
	{
		msgBackData.m_Flag = msgBackData.SUCCESS;
	}
	...
	{
		MemcatchXY::DeZhou::memUserDataInfo memUDI;
		Game::DeZhou::DBS::stUserDataInfo stUDI;

		if ( bHaveFindPlayer==false )
		{
			if ( m_memOperator.memGetUserDataInfo(msgLogin.m_AID,msgLogin.m_PID,memUDI) == false )
			{
				int ret = m_pDBOperator->ReadUserDataInfo(msgLogin.m_AID,msgLogin.m_PID,stUDI);
				...
				{
					pPlayerInfo->m_stUserDataInfo = stUDI;
				}
			}
			else
			{
				pPlayerInfo->m_stUserDataInfo.m_HaveGameInfo    = memUDI.m_HaveGameInfo;
		...
		if ( pPlayerInfo->m_stUserDataInfo.m_HaveGameInfo == 0 )//创建新玩家
		{
			bCreatePlayer = true;
			assert(memUDI.m_HaveGameInfo==0);

			BYTE nPlayerLevel = UINT8(pPlayerInfo->m_stUserDataInfo.m_PlayerLevel);
			if ( nPlayerLevel<=0 || nPlayerLevel>Max_Player_Level )
			{
				DebugError("OnReqPlayerLogin nPlayerLevel=%d",nPlayerLevel);
				nPlayerLevel = 1;
			}

			nOriginMoney = 50000;
			nLandMoney   = 5000;
			MapAreaInfo::iterator itorAI = m_mapAreaInfo.find(msgLogin.m_AID);
			if ( itorAI != m_mapAreaInfo.end() )
			{
				nOriginMoney = itorAI->second.m_JoinMoney[nPlayerLevel-1];
				nLandMoney   = itorAI->second.m_LandMoney[nPlayerLevel-1];
			}
			else
			{
				DebugError("Create LandMoney AID=%d",msgLogin.m_AID);
			}
			
			DezhouLib::stCreatePlayerGameInfo stCPGI;
			stCPGI.m_AID           = msgLogin.m_AID;
			stCPGI.m_PID           = msgLogin.m_PID;
			DezhouLib::GetCreatePlayerGameInfo(nPlayerLevel,stCPGI);
			...
			if ( msgBackData.m_Flag == msgBackData.SUCCESS )
			{
				...
				pPlayerInfo->m_stGameInfo.m_Right        = stCPGI.m_Right;
				pPlayerInfo->m_RightInfo.InitRight(stCPGI.m_Right);

				RWDB_CreateGameInfo rwdb_msgCGI;
				rwdb_msgCGI.m_AID     = msgLogin.m_AID;
				rwdb_msgCGI.m_PID     = msgLogin.m_PID;
				m_RWDBMsgManager.PushRWDBMsg(rwdb_msgCGI);

				AddPlayerMoneyLog(msgLogin.m_PID,nOriginMoney+nLandMoney,"CreatePlayer");

				if ( pPlayerInfo->m_stUserDataInfo.m_InvitePID > 0 )   //给予邀请人奖励
				{
					UINT32 InvitePID = pPlayerInfo->m_stUserDataInfo.m_InvitePID;
					INT16  InviteAID = pPlayerInfo->m_stUserDataInfo.m_InviteAID; ///*GetAIDByPID(InvitePID);*/

					RWDB_AddPlayerInfo msgAPI;
					msgAPI.m_AID          = InviteAID;
					msgAPI.m_PID          = InvitePID;
					msgAPI.m_Flag         = msgAPI.InvitePlayer;
					msgAPI.m_AddValue     = 1;
					m_RWDBMsgManager.PushRWDBMsg(msgAPI);

					RWDB_AddPlayerAward msgAPA;
					msgAPA.m_AID          = InviteAID;
					msgAPA.m_PID          = InvitePID;
					msgAPA.m_Money        = m_DBConfig.m_InviteAward;
					msgAPA.m_nFlag        = N_IncomeAndPay::InviteAward;
					msgAPA.m_PayMode      = PM_OneTime;
					msgAPA.m_CanUse       = 1;
					msgAPA.m_EndTime      = m_CurTime+N_Time::Month_1;
	...
	if ( msgBackData.m_Flag == msgBackData.SUCCESS )
	{
		Game::DeZhou::DBS::stUserGameInfo stUGI;
		if ( bHaveFindPlayer == false && bCreatePlayer == false )
		{
			...
			else if ( stUGI.m_stGameInfo.m_Forbid == 1 )
			{
	...
	if ( msgBackData.m_Flag == msgBackData.SUCCESS )
	{
		...
		{
			GetPlayerRight(pPlayerInfo);
			DebugLog("Login PID=%d RightLevel=%d RightTimes=%d",
				msgLogin.m_PID,pPlayerInfo->m_RightInfo.GetRightLevel(),pPlayerInfo->m_RightInfo.GetRightTimes() );
		}
	}
	...
	if ( msgBackData.m_Flag == msgBackData.SUCCESS )
	{
		...
		pPlayerInfo->m_bOnLine = true;
		pPlayerInfo->SetActiveTime(m_CurTime);
		if ( msgLogin.m_AID == N_AREAID::AREAID_VISITOR )
			pPlayerInfo->m_PlayerType = PLAYER_TYPE_VISITOR;
		else
	...
	if ( msgBackData.m_Flag == msgBackData.SUCCESS )  //将玩家的数据发送给玩家
	{
		...
		{
			VectorUserProduct vectorUP;
			if ( m_pDBOperator->ReadUserProduct(msgLogin.m_AID,msgLogin.m_PID,vectorUP) != DB_RESULT_SUCCESS )
			{
				DebugError("ReadUserProduct AID=%d PID=%d",msgLogin.m_AID,msgLogin.m_PID);
			}
			if ( vectorUP.size() )
			{
				DBServerXY::DBS_To_GS_Flag msgGSF;
				...
				for ( size_t nPos=0;nPos<vectorUP.size();nPos++)
				{
					pPlayerInfo->AddUserProduct(vectorUP[nPos]);
				}
			}
		}
	}
...
void CServer::DoPlayerHuiYuan(const DBS::stUserProduct& stUP,const DBS::stHuiYuanInfo& stHY)
{
	...
	if ( pPlayerInfo )
	{
		...
		if ( stHY.m_BankBox == 1 )
		{
			pPlayerInfo->m_stGameInfo.m_OpenBank = 1;

			RWDB_AddPlayerInfo rwdbmsgAPI;
			rwdbmsgAPI.m_AID        = pPlayerInfo->m_AID;
			rwdbmsgAPI.m_PID        = pPlayerInfo->m_PID;
			rwdbmsgAPI.m_Flag       = rwdbmsgAPI.OpenBank;
			rwdbmsgAPI.m_AddValue   = pPlayerInfo->m_stGameInfo.m_OpenBank;
			m_RWDBMsgManager.PushRWDBMsg(rwdbmsgAPI);
		}
		...
		DBS_msgUHY.m_FreeFaceTime   = pPlayerInfo->m_stGameInfoEX.m_FreeFaceTime;
		DBS_msgUHY.m_OpenBank       = BYTE(pPlayerInfo->m_stGameInfo.m_OpenBank);
		DBS_msgUHY.m_VipLevel       = BYTE(pPlayerInfo->m_stGameInfoEX.m_VipLevel);
		DBS_msgUHY.m_VipEndTime     = pPlayerInfo->m_stGameInfoEX.m_VipEndTime;
...
void CServer::SendPlayerGameData(CDBServerSocket* pSocket,INT16 AID,UINT32 PID)
{
	...
	if ( pPlayerInfo )
	{
		...
		msgPD.m_AID = AID;
		msgPD.m_PID = PID;
		msgPD.m_nBankMoney       = pPlayerInfo->m_stGameInfo.m_BankMoney;
		msgPD.m_OpenBank         = BYTE(pPlayerInfo->m_stGameInfo.m_OpenBank);
		msgPD.m_nGameMoney       = pPlayerInfo->m_stGameInfo.m_nGameMoney;
		msgPD.m_nMoGuiMoney      = pPlayerInfo->m_stGameInfo.m_nMoGuiMoney;
		msgPD.m_nMatchJF         = pPlayerInfo->m_stGameInfo.m_nMatchJF;
		...
		msgPD.m_Sex               = BYTE(pPlayerInfo->m_stUserDataInfo.m_Sex);
		msgPD.m_ChangeName        = BYTE(pPlayerInfo->m_stUserDataInfo.m_ChangeName);
		...
		msgPDEx.m_AID            = AID;
		msgPDEx.m_PID            = PID;
		msgPDEx.m_MaxMoney       = pPlayerInfo->m_stGameInfoEX.m_MaxMoney;
		msgPDEx.m_MaxMoneyTime   = pPlayerInfo->m_stGameInfoEX.m_MaxMoneyTime;
		msgPDEx.m_MaxPai         = pPlayerInfo->m_stGameInfoEX.m_MaxPai;
		msgPDEx.m_MaxPaiTime     = pPlayerInfo->m_stGameInfoEX.m_MaxPaiTime;
		msgPDEx.m_MaxWin         = pPlayerInfo->m_stGameInfoEX.m_MaxWin;
		msgPDEx.m_MaxWinTime     = pPlayerInfo->m_stGameInfoEX.m_MaxWinTime;
		memcpy(msgPDEx.m_Achieve,pPlayerInfo->m_stGameInfoEX.m_Achieve,sizeof(msgPDEx.m_Achieve));
		memcpy(msgPDEx.m_WinRecord,pPlayerInfo->m_stGameInfoEX.m_WinRecord,sizeof(msgPDEx.m_WinRecord));

		msgPDEx.m_FreeFaceTime   = pPlayerInfo->m_stGameInfoEX.m_FreeFaceTime;
		msgPDEx.m_VipLevel       = pPlayerInfo->m_stGameInfoEX.m_VipLevel;

		msgPDEx.m_SendGift       = pPlayerInfo->m_stGameInfoEX.m_SendGift;
		msgPDEx.m_RecvGift       = pPlayerInfo->m_stGameInfoEX.m_RecvGift;
		msgPDEx.m_ShowFace       = pPlayerInfo->m_stGameInfoEX.m_ShowFace;
...
int CServer::OnReqPlayerInfo( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	{
		DBServerXY::DBS_PlayerData msgPD;
		msgPD.m_AID = msgPI.m_AID;
		msgPD.m_PID = msgPI.m_PID;
	...
	if ( (msgPI.m_DataFlag & UINT16(msgPI.PlayerDataEx)) && msgRespPI.m_DataFlag == 0 )
	{
		...
		msgPDEx.m_AID            = msgPI.m_AID;
		msgPDEx.m_PID            = msgPI.m_PID;
...
int CServer::OnReqAddBotMoney( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	{		
			...
			&& pPlayerInfo->m_PID == msgABM.m_PID && pPlayerInfo->m_BotLevel == msgABM.m_nLevel )
		{
			...
			INT64 MaxMoney = DezhouLib::GetBotLevelMoney(BotLevel);
			INT64 MinMoney = DezhouLib::GetBotLevelMoney(BotLevel-1);
			...
			if ( pPlayerInfo->m_stGameInfo.m_nGameMoney == msgABM.m_nGameMoney )
			{
				...
				if ( nAddGameMoney != 0 )
				{
					...
					RWDB_UpdateUserGameMoneyType rwdbmsgUGMT;
					rwdbmsgUGMT.m_AID         = msgABM.m_AID;
					rwdbmsgUGMT.m_PID         = msgABM.m_PID;
					rwdbmsgUGMT.m_nGivenMoney = nAddGameMoney;
					m_RWDBMsgManager.PushRWDBMsg(rwdbmsgUGMT);
...
int CServer::UpdatePlayerRight(DBServerPlayerInfo* pPlayerInfo)
{
	...
	if ( pPlayerInfo )
	{
		int OldLevel = pPlayerInfo->m_RightInfo.GetRightLevel();
		pPlayerInfo->m_RightInfo.ReduceRightTime();
		if ( OldLevel != pPlayerInfo->m_RightInfo.GetRightLevel() )
		{
			DBServerXY::DBS_PlayerRight msgPR;
			msgPR.m_AID    = pPlayerInfo->m_AID;
			msgPR.m_PID    = pPlayerInfo->m_PID;
			msgPR.m_Level  = pPlayerInfo->m_RightInfo.GetRightLevel();
			msgPR.m_Times  = pPlayerInfo->m_RightInfo.GetRightTimes();
			SendMsgToPlayer(msgPR,pPlayerInfo->m_PID);
		}
	}
	return 0;
}
	...
	if ( wl.m_PID > 0 )
	{
		...
		if ( pPlayerInfo )
		{
			...
			if ( wl.m_EndGameFlag == TALLY_WIN )
			{
				...
				if ( DezhouLib::IsValidPaiType(wl.m_PaiType) )
				{
					pPlayerInfo->m_stGameInfoEX.m_WinRecord[wl.m_PaiType-1]++;
				}
			}
			else
			{
...
int CServer::OnWDBMatchResult( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	if ( msgMR.m_PID>0 && msgMR.m_AID>0 )
	{
		...
		RWDB_UpdateUserGameMoneyType dbmsg_UUGM;
		dbmsg_UUGM.m_AID           = msgMR.m_AID;
		dbmsg_UUGM.m_PID           = msgMR.m_PID;
		dbmsg_UUGM.m_nServiceMoney = msgMR.m_Ticket;
		dbmsg_UUGM.m_nLossMoney    = msgMR.m_TakeMoney;
		if ( msgMR.m_TakeMoney > 0 )
		{
			dbmsg_UUGM.m_nWinMoney     = msgMR.m_AwardMoney;
		}
		else
		{
			dbmsg_UUGM.m_nGivenMoney   = msgMR.m_AwardMoney;
		}		
		m_RWDBMsgManager.PushRWDBMsg(dbmsg_UUGM);
...
int CServer::OnWDBIncomeAndPay( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	if ( msgIP.m_AID>0 && msgIP.m_PID>0 )
	{
		...
		RWDB_UpdateUserGameMoneyType rwdbmsgUGMT;
		rwdbmsgUGMT.m_AID           = msgIP.m_AID;
		rwdbmsgUGMT.m_PID           = msgIP.m_PID;
		if ( DezhouLib::IsIncomeFlag(msgIP.m_Flag) )
		{
			rwdbmsgUGMT.m_nGivenMoney = msgIP.m_nMoney;
		}
		else if ( DezhouLib::IsPayFlag(msgIP.m_Flag) )
		{
			rwdbmsgUGMT.m_nExpendMoney = -msgIP.m_nMoney;
		}
		m_RWDBMsgManager.PushRWDBMsg(rwdbmsgUGMT);
...
int CServer::OnWDBChangeBankMoney( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	TransplainMsg(msgPack,msgCBM);

	DebugInfo("OnWDBChangeBankMoney Start AID=%d PID=%d Flag=%d Money=%s",
		msgCBM.m_AID,msgCBM.m_PID,msgCBM.m_Flag,Tool::N2S(msgCBM.m_nMoney.get()).c_str());

	if ( msgCBM.m_AID>0 && msgCBM.m_PID>0 && msgCBM.m_nMoney>0 )
	{
		INT64 nGameMoney = 0;
		INT64 nBankMoney = 0;

		RWDB_ChangeBankLog msgCBL;
		msgCBL.m_AID      = msgCBM.m_AID;
		msgCBL.m_PID      = msgCBM.m_PID;
		msgCBL.m_Money    = msgCBM.m_nMoney;
		msgCBL.m_Flag     = msgCBM.m_Flag;
		m_RWDBMsgManager.PushRWDBMsg(msgCBL);

		if ( msgCBM.m_Flag == msgCBM.BankToGame )
		{
			RWDB_IncomeAndPay msgIP;
			msgIP.m_AID       = msgCBM.m_AID;
			msgIP.m_PID       = msgCBM.m_PID;
			msgIP.m_nMoney    = msgCBM.m_nMoney;
			msgIP.m_Flag      = N_IncomeAndPay::BankToGame;
			m_RWDBMsgManager.PushRWDBMsg(msgIP);

			nGameMoney = msgCBM.m_nMoney;
			nBankMoney = -msgCBM.m_nMoney;
		}
		else if ( msgCBM.m_Flag == msgCBM.GameToBank )
		{
			RWDB_IncomeAndPay msgIP;
			msgIP.m_AID       = msgCBM.m_AID;
			msgIP.m_PID       = msgCBM.m_PID;
			msgIP.m_nMoney    = -msgCBM.m_nMoney;
			msgIP.m_Flag      = N_IncomeAndPay::GameToBank;
			m_RWDBMsgManager.PushRWDBMsg(msgIP);
		...
		DBServerPlayerInfo* pPlayerInfo = GetDBPlayerInfo(msgCBM.m_PID);
		if ( pPlayerInfo )
		{
			pPlayerInfo->m_stGameInfo.m_nGameMoney += nGameMoney;
			pPlayerInfo->m_stGameInfo.m_BankMoney  += nBankMoney;

			m_pDBOperator->UpdateChangeBankMoney(msgCBM.m_AID,msgCBM.m_PID,nGameMoney,nBankMoney);
		}
		else
		{
			DebugError("OnWDBChangeBankMoney PID=%d Flag=%d Money=%s",msgCBM.m_PID,msgCBM.m_Flag,N2S(msgCBM.m_nMoney.get()).c_str());
		}
	}
	DebugInfo("OnWDBChangeBankMoney End");
	return 0;
}
int CServer::OnWDBMaxPai( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	DebugInfo("CServer::OnWDBMaxPai Start ServerID=%d,AID=%d PID=%d MaxPai=%s",pSocket->GetServerID(),
		msgMP.m_AID,msgMP.m_PID,Tool::N2S(msgMP.m_MaxPai).c_str() );
	...
	if ( pPlayerInfo )
	{
		pPlayerInfo->m_stGameInfoEX.m_MaxPai     = msgMP.m_MaxPai;
		pPlayerInfo->m_stGameInfoEX.m_MaxPaiTime = msgMP.m_MaxPaiTime;
	}
	...
	if ( msgMWL.m_PID )
	{
		...
		if ( pPlayerInfo )
		{
			...
			if ( msgMWL.m_nWinLossMoney > 0 )
			{
				...
				pPlayerInfo->m_stGameInfo.m_nWinTimes++;
				if ( DezhouLib::IsValidPaiType(msgMWL.m_PaiType) )
				{
					pPlayerInfo->m_stGameInfoEX.m_WinRecord[msgMWL.m_PaiType-1]++;
				}
			}
			else
			{
...
int CServer::OnWDBFinishHonor( CDBServerSocket* pSocket,CRecvMsgPacket& msgPack )
{
	...
	TransplainMsg(msgPack,msgFH);

	DebugInfo("OnWDBFinishHonor Start AID=%d PID=%d HonrID=%d",msgFH.m_AID,msgFH.m_PID,msgFH.m_HonorID);

	DBServerPlayerInfo* pPlayerInfo = GetDBPlayerInfo(msgFH.m_PID);
	if ( pPlayerInfo && DezhouLib::IsValidHonorID(msgFH.m_HonorID) )
	{
		BYTE nType = DezhouLib::GetType(msgFH.m_HonorID);
		BYTE nIdx  = DezhouLib::GetIdx(msgFH.m_HonorID);
		if ( DezhouLib::IsValidType(nType) && DezhouLib::IsValidIdx(nIdx) )
		{
			if ( (pPlayerInfo->m_stGameInfoEX.m_Achieve[nType-1]&S_Uint32_Number[nIdx-1]) == 0 )
			{
				pPlayerInfo->m_stGameInfoEX.m_Achieve[nType-1] |= S_Uint32_Number[nIdx-1];

				RWDB_UpdateAchieve rwdbUA;
				rwdbUA.m_AID        = msgFH.m_AID;
				rwdbUA.m_PID        = msgFH.m_PID;
				rwdbUA.m_HonorType  = nType;
				rwdbUA.m_HonorValue = S_Uint32_Number[nIdx-1];
				m_RWDBMsgManager.PushRWDBMsg(rwdbUA);

				MapHonorInfo::iterator itorHI = m_mapHonorInfo.find(msgFH.m_HonorID);
				if ( itorHI != m_mapHonorInfo.end() )
				{
					INT64 nAddMoney = itorHI->second.m_HonorMoney;

					AddPlayerMoneyLog(pPlayerInfo->m_PID,nAddMoney,"OnWDBFinishHonor");
					AddUserGameMoney(pPlayerInfo->m_AID,pPlayerInfo->m_PID,nAddMoney);				

					DBServerXY::WDB_RespFinishHonor respFH;
					respFH.m_AID         = msgFH.m_AID;
					respFH.m_PID         = msgFH.m_PID;
					respFH.m_HonorID     = msgFH.m_HonorID;
					respFH.m_HonorMoney  = nAddMoney;
					pSocket->SendMsg(respFH);					

					RWDB_HonorLog rwdgHL;
					rwdgHL.m_AID        = msgFH.m_AID;
					rwdgHL.m_PID        = msgFH.m_PID;
					rwdgHL.m_HonorID    = msgFH.m_HonorID;
					rwdgHL.m_Money      = itorHI->second.m_HonorMoney;
					m_RWDBMsgManager.PushRWDBMsg(rwdgHL);
				}
				else
				{
					DebugError("OnWDBFinishHonor AID=%d PID=%d HonorID=%d",msgFH.m_AID,msgFH.m_PID,msgFH.m_HonorID);
				}
			}			
		}
	}

	DebugInfo("OnWDBFinishHonor End");
