...
namespace MoGui
{
	namespace MoGuiXY
	{
		...
		/*
		const short CMDT_FIRST_CLIENT_AD			= CMDT_FIRST_CLIENT+4100;	//广告协议起始编号
		const short CMDT_LAST_CLIENT_AD				= CMDT_FIRST_CLIENT+4499;	//广告协议结束编号

		const short CMDT_FIRST_CLIENT_ISP			= CMDT_FIRST_CLIENT+4500;	//ISP充值协议起始编号
		const short CMDT_LAST_CLIENT_ISP			= CMDT_FIRST_CLIENT+4599;	//ISP充值协议结束编号

		const short CMDT_FIRST_CLIENT_PT			= CMDT_FIRST_CLIENT+4600;	//pt协议起始编号
		const short CMDT_LAST_CLIENT_PT				= CMDT_FIRST_CLIENT+4649;	//pt协议结束编号

		const short CMDT_FIRST_CLIENT_PIC			= CMDT_FIRST_CLIENT+4650;	//图片协议起始编号
		const short CMDT_LAST_CLIENT_PIC			= CMDT_FIRST_CLIENT+4699;	//图片协议结束编号

		const short CMDT_FIRST_CLIENT_AREARS		= CMDT_FIRST_CLIENT+4700;	//区域化人数监控
		const short CMDT_LAST_CLIENT_AREARS			= CMDT_FIRST_CLIENT+4749;	//区域化人数监控

		const short CMDT_FIRST_CLIENT_AREATGY		= CMDT_FIRST_CLIENT+4750;	//区域化推广员系统
		const short CMDT_LAST_CLIENT_AREATGY		= CMDT_FIRST_CLIENT+4799;	//区域化推广员系统
		*/
	}
}