...
namespace MoGui
{
	const int InfoFlag_ChangName       = 1;
	const int InfoFlag_GameInfo        = 2;
	...
	{
		Login_Type_None = 0,
		Login_Type_UserName = 11,
		Login_Type_Session = 21,
		Login_Type_Relink = 31,
		Login_Type_HardCode = 41,
	...
	namespace N_IncomeAndPay
	{
		...
		const short   InPay_Max       = 4000;


		bool IsValidPayIncomeFlag(int nFlag)
		{
			if ( (nFlag>0&&nFlag<2000) || (nFlag>2000&&nFlag<4000) )
			{
				return true;
			}
			return false;
