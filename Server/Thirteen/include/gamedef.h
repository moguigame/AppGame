...
namespace MoGui
{
	namespace Game
	{
		...
		namespace DeZhou
		{
			const short    MAX_PALYER_ON_TABLE      = 4;                   //本桌最多一起游戏人数
			const short    MIN_PLAYER_FOR_GAME      = 4;                   //开始游戏的最少玩家
			...
			enum ROOMTYPE
			{
				ROOM_TYPE_Common             = 1,         //桌间模式是普通的，玩家自己选择房间桌子位置和坐位，
				ROOM_TYPE_TableMatch         = 2,         //单桌比赛,
				ROOM_TYPE_RoomMatch          = 3,         //整个房间的比赛，主要是组织大型比赛
				ROOM_TYPE_AutoSit            = 4,         //排队性质的，由系统自动给玩家分配位置
			};
