...
int CDBOperator::ReadUserGameInfo(INT16 AID,UINT32 PID,stDBGameInfo& stUGI)
{
...
//	return ret;
//}
int CDBOperator::ReadAreaInfo(VectorDBAreaInfo& vectorArea)
{
	...
	try
	{
		...
		stDBAreaInfo stAI;
		otl_stream TempDBStream(OTL_STREAMBUF_SIZE,"select AID,AreaName,JoinMoney,LandMoney from dezhou_info_area ",m_DBConfigConnect);
		while ( !TempDBStream.eof() )
		{
			stAI.Init();
...
int CDBOperator::ReadUserGiftInfo(INT16 AID,UINT32 PID,VectorDBUserGiftInfo& vectorUGI)
{
	...
	try
	{
		...
		std::string strSQL = "select Idx,SendPID,GiftID,Price,Name,CreateTime from " + getGiftTableName(PID) + 
			" where RecvPID=:f1<unsigned int> and Flag=:f2<short>";
		otl_stream TempDBStream( OTL_STREAMBUF_SIZE,strSQL.c_str(),m_DBPropConnect );
		TempDBStream<<PID<<short(N_Gift::GiftST_OnSale);
...
int CDBOperator::CreatePlayerGameInfo(stCreateGameInfo& stCPGI)
{
	...
	{
		{
			...
			string strGameInfo = "insert into user_money (PID,AID,GameMoney) \
								 select :f1<int>,:f2<int>,:f11<bigint> \
								 from dual where not exists \
								 (select * from dezhou_money where user_money.PID = :f11<int>)";
		...
		{
			string strGameInfo = "insert into thirteen_gameinfo (PID,AID) select :f1<int>,:f2<int> \
								 from dual where not exists \
								 (select * from dezhou_gameinfo where dezhou_gameinfo.PID = :f11<int>)";
			otl_stream GameInfoStream( OTL_STREAMBUF_SIZE,strGameInfo.c_str(),m_DBGameConnect );
...
int CDBOperator::RegisterUser(const stRegisterUser& stRU)
{
...
int CDBOperator::ReadUserDataInfoByPID(int PID,stDBUserDataInfo& stUDI)
{
	...
	try
	{
		string strSQL = "select PID,AID,Sex,UserName,Password,NickName,HardCode,HeadID,HeadPicURL,HomeURL,City,InvitePID \
...
int CDBOperator::ReadUserDataInfoByUserName(int AID,const string& UserName,stDBUserDataInfo& stUDI)
{
...
int CDBOperator::ReadUserDataInfoByHardCode(const string& HardCode,stDBUserDataInfo& stUDI)
{
